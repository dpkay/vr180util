...
class ResolveContext:
    def __init__(self, resolve):
        ...
        self.timelines_by_import_root_folder_path = "/timelines_v2/by_import"
    ...
    def GetFootageClipsInFolder(self, folder):
        return [
            ...
            if clip.GetClipProperty("type") in ["Video", "Video + Audio"]
        ]
...
class ResolveUpdater:
    ...
    def LinkProxyForMediaPoolItem(self, media_pool_item, proxy_root_filesystem_path):
        ...
        try:
            ...
            print(f"GetSequenceNameFromMediaPoolItem for shot {shot_name} failed.")
            return
        ...
        if not media_pool_item.LinkProxyMedia(proxy_filesystem_path):
            ...
            print(
                f"LinkProxyMedia for shot {sequence_name}/{shot_name} and proxy path succeeded: {proxy_filesystem_path}"
            )
    ...
    def CreateMissingShootSummaryTimelines(self):
        ...
        timeline_sequence_names = set(
            self.resolve_context.GetSubFolderNames(
                self.resolve_context.GetFolderByPath(
                    self.resolve_context.timelines_by_import_root_folder_path
                )
            )
        )
        ...
        timelines_by_import_root_folder = self.resolve_context.GetFolderByPath(
            self.resolve_context.timelines_by_import_root_folder_path
        )
        ...
        for sequence_name in missing_timeline_sequence_names:
            self.resolve_context.media_pool.AddSubFolder(
                timelines_by_import_root_folder, sequence_name
            )
    ...
    def MaybeCreateMissingShootSummaryTimeline(self, sequence_name):
        ...
        timeline_folder = self.resolve_context.GetFolderByPath(
            f"{self.resolve_context.timelines_by_import_root_folder_path}/{sequence_name}"
        )
        ...
        # print("seq?")
        # if sequence_name == "20240802":
        #     print(sequence_name)
        #     print(motion_magnitude_sequence_names)
        #     print("exiting")
        #     import sys
        #     sys.exit(1)
        ...
        if not timeline_name in self.resolve_context.GetClipNames(timeline_folder):
            ...
            print(f"mmc: {[x.GetName() for x in motion_magnitude_clips]}")
            print(f"fc: {[x.GetName() for x in footage_clips]}")
